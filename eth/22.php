...
foreach($result['blocks'] as $k=>$v){
	if($block<$v['number']) {
        $sql = "INSERT INTO blocks (id, miner, systemgentime, gentimestamp , ) values ('" . $v['number'] . "', '" . $v['miner'] . "', '" . $v['time'] . "', '" . strtotime($v['time']) . "', '" . $v['blocktime'] . "')";
        var_dump($sql);
...
/*
$result=json_decode($result,true);
krsort($result['curentStats']);
$curentStats=file_get_contents('curentStats');
foreach($result['curentStats'] as $k=>$v){
    if(curentstats<$v['time']){
        $sql="INSERT INTO curentStats (systemgentime, price_usd, price_btc, difficulty , hashrate, tps) values ('".strtotime($v['time'])."', '".$v['price_usd']."', '".$v['price_btc']."', '".$v['difficulty']."', '".$v['hashrate']."', '".$v['tps']."')";
        var_dump($sql);
        file_put_contents('curentStats',$v['time']);
    }
}
*/
var_dump( $result['blocks'] );
//var_dump( $result['curentStats'] );


$mysqli = new mysqli("localhost", "username", "password", "eth");
